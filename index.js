...
async function startXeonBotInc() {
    ...
    XeonBotInc.sendMessage(callerId, { text: `*Sistem pemblokiran otomatis!*\n*Jangan panggil bot*!\n*Harap hubungi Owner untuk membuka blokir !*`}, { quoted : blockxeon })
    await sleep(8000)
    ...
    // Get Profile Picture Group
       ...
       XeonBotInc.send5ButImg(pea[0].id, `「 Pengaturan Grup Diubah 」\n\n Grup Telah Ditutup Oleh Admin, Sekarang Hanya Admin Yang Dapat Mengirim Pesan!`, `${botname}`, lolXeon, [])
       } else if(pea[0].announce == false) {
       XeonBotInc.send5ButImg(pea[0].id, `「 Pengaturan Grup Diubah 」\n\nGrup Sudah Dibuka Admin, Sekarang Peserta Bisa Kirim Pesan !`, `${botname}`, lolXeon, [])
       } else if (pea[0].restrict == true) {
       XeonBotInc.send5ButImg(pea[0].id, `「 Pengaturan Grup Diubah 」\n\nInfo Grup Telah Dibatasi, Sekarang Hanya Admin Yang Dapat Mengedit Info Grup !`, `${botname}`, lolXeon, [])
       } else if (pea[0].restrict == false) {
       XeonBotInc.send5ButImg(pea[0].id, `「 Pengaturan Grup Diubah 」\n\nInfo Grup Telah Dibuka, Sekarang Peserta Bisa Edit Info Grup !`, `${botname}`, lolXeon, [])
       } else {
       XeonBotInc.send5ButImg(pea[0].id, `「 Pengaturan Grup Diubah 」\n\nNama group diganti menjadi *${pea[0].subject}*`, `${botname}`, lolXeon, [])
     }
    })
...
└┬❖ 「 @${xeonName.split("@")[0]}  」
   ...
   ❖ Patuhi rules yang dibuat admin
   ❖ Semoga betah
   └───────────────┈ ⳹`
let buttons = [
...
title: `${ownername}`,
body: `Jangan lupa baca deskripsi group ya!`,
mediaType:2,
thumbnail: XeonWlcm,
sourceUrl: `${websitex}`,
mediaUrl: `${websitex}`
...
XeonBotInc.sendMessage(anu.id, buttonMessage, {quoted:unicorndoc})
    ...
    XeonBotInc.getName = (jid, withoutContact  = true) => {
	...
	for (let i of kon) {
	    list.push({
	    	...
	    	vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await XeonBotInc.getName(i)}\nFN:${await XeonBotInc.getName(i)}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Ini Ownerku kak, Jangan macam-macam lo ya!\nitem2.EMAIL;type=INTERNET: aliyulmanan@gmail.com\nitem2.X-ABLabel: Email\nitem3.URL: https://instagram.com/malasnulisnamapengguna\nitem3.X-ABLabel:Instagram\nitem4.ADR:;;${location};;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
	    })
	}
    ...
    XeonBotInc.ev.on('connection.update', async (update) => {
        ...
        let reason = new Boom(lastDisconnect?.error)?.output.statusCode
            ...
            else if (reason === DisconnectReason.connectionClosed) { console.log("Koneksi ditutup, menghubungkan kembali...."); startXeonBotInc(); }
            else if (reason === DisconnectReason.connectionLost) { console.log("Koneksi Hilang dari Server, menyambungkan kembali..."); startXeonBotInc(); }
            else if (reason === DisconnectReason.connectionReplaced) { console.log("Koneksi Diganti, Sesi Baru Dibuka, Harap Tutup Sesi Saat Ini Terlebih Dahulu"); XeonBotInc.logout(); }
            else if (reason === DisconnectReason.loggedOut) { console.log(`Perangkat Keluar, Harap Pindai Lagi Dan Jalankan.`); XeonBotInc.logout(); }
            else if (reason === DisconnectReason.restartRequired) { console.log("Restart Diperlukan, Restart..."); startXeonBotInc(); }
            else if (reason === DisconnectReason.timedOut) { console.log("Koneksi Habis, Menghubungkan..."); startXeonBotInc(); }
